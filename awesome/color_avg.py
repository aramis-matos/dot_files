...
def color_avg(img_path: str, old_wallpaper: str, index: int):
    ...
    theme_lua_path = f"/home/ccyanide/.config/awesome/theme{index}.lua"
    with open(theme_lua_path,'r') as f:
        ...
        contents = replace_color(fg_normal,primaryCompliment,contents)   
        contents = replace_color(fg_focus,secondaryCompliment,contents)
...
with open(nitrogen_path,'r') as f:
    img_paths = [x.split("=")[1].strip() for x in re.findall(r"\[xin_\d\]\nfile=.*",f.read())]
img_paths.sort(key=lambda x: x)

old_wallpapers = [f"/home/ccyanide/.config/awesome/curr_wallpaper{x+1}.txt" for x in range(len(img_paths))]
sameWallpapers = True
for (old_wallpaper,img_path) in zip(old_wallpapers,img_paths):
    with open(old_wallpaper,'r') as f:
        if f.read().strip() != img_path:
            sameWallpapers = False

if sameWallpapers:
    exit(0)

for (index,(path,old_wallpaper)) in enumerate(zip(img_paths,old_wallpapers),1):
    color_avg(path,old_wallpaper,index)
